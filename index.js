...
function obtenerMejorDestino(listaPescados) {
    var pesoTotalPescados = 0;
    ...
    listaPescados.forEach(function (pesc) {
        ciudades.forEach(function (ciudad) {
            beneficios.filter(function (ben) { return ben.nombreCiudad === ciudad.nombreCiudad.toLowerCase(); })[0].beneficioTotal += ciudad.preciosPescados[pesc.nombrePescado.toLowerCase()] * pesc.cantidad;
            //beneficios[ciudad.ciudad.toLowerCase()]+= ciudad.precios[pesc.pescado.toLowerCase()]*pesc.cantidad;
        });
    });
    ciudades.forEach(function (ciud) {
        var distanciaKm = ciud.distanciaKm;
        var depreciacionPescado = 1 - (distanciaKm / 100) / 100;
        var costeFurgo = 5 + distanciaKm * 2;
        //beneficios[ciud.ciudad.toLowerCase()] = beneficios[ciud.ciudad.toLowerCase()]*depreciacion - costeFurgo;
        var beneficioCiudadX = beneficios.filter(function (ben) { return ben.nombreCiudad === ciud.nombreCiudad.toLowerCase(); })[0];
        beneficios.filter(function (ben) { return ben.nombreCiudad === ciud.nombreCiudad.toLowerCase(); })[0].beneficioTotal = beneficioCiudadX.beneficio * depreciacionPescado - costeFurgo;
    });
